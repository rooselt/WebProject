...
namespace TCC.Infra.Mapping
{
    public class ColaboradorMap : BaseMap<Colaborador>
    {
        public ColaboradorMap()
        {
            ...
            this.Property(x => x.Valor).HasColumnName("VALOR").IsOptional();
            ...
            this.Property(x => x.DataAdmissao).HasColumnName("DATA_ADMISSAO").IsRequired();
            this.Property(x => x.IdArqFoto).HasColumnName("ID_ARQ_FOTO")
                  .HasColumnAnnotation("Index", new IndexAnnotation(new IndexAttribute("IX_ID_ARQ_FOTO") { IsUnique = false })).IsOptional();
